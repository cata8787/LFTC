...
BST* createTS(vector<string> words, unordered_map<string, int> table, AF af_constante, AF af_identificatori) {
    BST* ts = new BST();
    ...
    for (string word : words) {
        ...
        if (table.find(word) == table.end()) // daca nu este cuvant rezervat atunci 
            if (af_constante.verifica(word) || af_identificatori.verifica(word)) // verific daca e id sau constanta
...
vector<FIP>* createFIP(vector<string> words, unordered_map<string, int> table, BST* ts, AF af_constante, AF af_identificatori) {
    ...
    for (string word : words) {
        ...
        // altfel trebuie sa vad daca este ID sau CONST si sa il adaug in FIP
            ...
            if (af_constante.verifica(word))
                fip->push_back(FIP(table.at("CONST"), ts->search(word)));
            else {
                if (af_identificatori.verifica(word))
...
int main()
{
    AF af_constante = AF("AF_CONSTANTE.txt");
    AF af_identificaori = AF("AF_IDENTIFICATORI.txt");

    vector<string> words = readFile("input.txt");

    unordered_map<string, int> table = createTable(); // dictionar auxiliar cu simboluri
    BST* ts = createTS(words, table, af_constante, af_identificaori); // ts
    vector<FIP>* fip = createFIP(words, table, ts, af_constante, af_identificaori);  //fip
    ...
    writeFile("forma_interna_a_programului.txt", descris);
